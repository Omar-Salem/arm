...
mkdir -p ~/arm_ws/src
cd ~/arm_ws/src 
git clone git@github.com:Omar-Salem/arm.git .
cd ~/arm_ws
colcon build 
````
...
```bash
cd ~/arm_ws && colcon build --packages-select arm_description && source install/setup.bash && ros2 launch arm_description display.launch.py use_gui:=true use_joint_state_publisher:=true
```
...
### Moveit
```bash
cd ~/arm_ws && colcon build --packages-select arm_mapping && source install/setup.bash && ros2 launch arm_mapping gazebo.launch.py
```
